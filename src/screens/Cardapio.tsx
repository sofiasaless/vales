...
export default function Cardapio() {
  ...
  const handleAdicionarItems = async () => {
    ...
    await adicionarVales.mutateAsync({
      props: {
        id: idFunc,
  ...
  useEffect(() => {
    if (adicionarVales.isPending) return;
    if (adicionarVales.isSuccess) {
      limparItens();
      eventoNovaAdicaoVale();
      navigation.goBack();
    }
    if (adicionarVales.isError) {
      alert(
        "Ocorreu um erro ao adicionar os vales",
        adicionarVales.error?.message,
      );
    }
  }, [adicionarVales.isPending]);
    ...
    <Layout style={styles.container}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ItemCardapio product={item} gerente={gerente!} />
          )}
          ListEmptyComponent={
